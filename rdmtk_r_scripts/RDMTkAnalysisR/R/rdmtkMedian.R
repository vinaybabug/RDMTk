rdmtkMedian<-function(taskType, exprID, ...){
  library(DBI);
  ...
  if(taskType == "BART"){
    ...
    dbClearResult(res);
    med_res<-median(as.numeric(tempRslt$total_pts));
    ...
    tempRslt <- dbFetch(res); 
    dbClearResult(res);
    med_res<-median(as.numeric(tempRslt$final_total));
    ...
    tempRslt <- dbFetch(res); 
    dbClearResult(res);
    med_res<-median(as.numeric(tempRslt$score));
