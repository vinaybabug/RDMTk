rdmtkIGTRandomModel <- function(taskType, exprID, ...) {
  ...
  outputmat=matrix(c(subjnumvec,numtrialsvec,g2rand),nrow=numsubj,ncol=3)

  colnames(outputmat)=c("subject","numtrials","g2rand")
  ...
  SQL <- paste("SELECT EXISTS(SELECT * FROM expr_anlys_data WHERE experid ='", exprID, "' and expertype = '", taskType,"' and anlys_mdl = 'RND_MDL');", sep = "");
  ...
  if(tempRslt[1] == 0){
    ...
    VALUES ( '%s', '%s', '%s', '%s', '%s', '%s');", exprID, taskType, 'RND_MDL', txt_binary, "R", "R");
    ...
    SQL <- sprintf("UPDATE `rdmtoolkit`.`expr_anlys_data`
                    ...
                    `anlys_rslt` = '%s',
                    `modified_by` = '%s'
                    WHERE `experid` = '%s' AND `expertype` = '%s' AND `anlys_mdl` = '%s';", txt_binary, "R", exprID, taskType, 'RND_MDL');
    res <- dbSendQuery(con, SQL);
    dbClearResult(res);
  ...
  #returnList <- list("loglikerand" = loglikerand, "g2rand" = g2rand, "subjnumvec" = subjnumvec);
  #return(returnList);
}
