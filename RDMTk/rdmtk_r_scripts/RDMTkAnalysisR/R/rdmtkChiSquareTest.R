rdmtkChiSquareTest<-function(taskType, exprID, ...){
  library(DBI);
  ...
  if(taskType == "BART"){
    ...
    dbClearResult(res);
    chi_res<-chisq.test(as.numeric(tempRslt$trial_pts), as.numeric(tempRslt$total_pts));
  }
  else
    if(taskType == "CUPS"){
    ...
    tempRslt <- dbFetch(res); 
    dbClearResult(res);
    chi_res<-chisq.test(as.numeric(tempRslt$initial_total), as.numeric(tempRslt$final_total));
    ...
    tempRslt <- dbFetch(res); 
    dbClearResult(res);
    chi_res<-chisq.test(as.numeric(tempRslt$score), as.numeric(tempRslt$response));
