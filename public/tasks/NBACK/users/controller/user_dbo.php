	...
	public function viewFieldsParticipantCond($fields,$cond)
     {
		...
		$db = new OE_DataBaseManager();
		$db->connect();
		$db->sql('SELECT '.$fields.' FROM nback_expr_data where '.$cond);
		$res = $db->getResult();
                $db->disconnect();
		return json_encode($res); 
     }	
	 public function listdata($fields,$cond)
     {
		...
		$db = new OE_DataBaseManager();
		$db->connect();
		$db->sql('SELECT '.$fields.' FROM nback_score_practice '.$cond);
		$res = $db->getResult();
		return json_encode($res); 
     }
 public function listdata_main($fields,$cond)
     {
	 
		$db = new OE_DataBaseManager();
		$db->connect();
		$db->sql('SELECT '.$fields.' FROM nback_score_values '.$cond);
		$res = $db->getResult();
		return json_encode($res); 
     }	 
