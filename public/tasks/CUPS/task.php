...
ob_start();
	...
	if(($countexists > 0)&&($participantid!==""))
	{
		...
		$viewexpts= $userdbo->viewFieldsParticipantCond('total_pts','mid="'.$participantid.'" and experid="'.$experimentid.'" order by trialno');
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="src/css/cupgame.css">
				...
				<script>
					...
					data_trial[0] = new Array(2);
					data_trial[0][0]=999;
					data_trial[0][1]=999;
					
					for(i=1; i<=45;i++){
						var j = Math.floor((i - 1) / 5) + 1;		
					...
					//alert(data_trial.length);
					
					for(i=1; i<data_trial.length;i++){
						if(data_trial[i][0] == 999)
							data_trial[i]=data_trial[0];
					}
					
					/*for(i=1; i<data_trial.length;i++){
					alert("["+i+"] "+"-cups-# "+ data_trial[i][0]+ " -pay-# " + data_trial[i][1]);
					}*/
					...
					//var size = ["2","3","5"];
					var size_rand = data_trial[1][0];
					var amount_rand = data_trial[1][1];
					var color_rand = "red";
					if(data_trial[1][1] > 0){
						color_rand = "blue";
					}
